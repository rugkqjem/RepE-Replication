...
def get_truncated_outputs(all_outputs,instructions,num_examples,user_tag,assistant_tag,pos_type,neg_type,control_template):
    ...
    for s,p in zip(all_outputs,instructions):
        ...
        pos_s.append(pos_template.format(user_tag=user_tag,assistant_tag=assistant_tag,instruction=p,type=control_template.format(type=pos_type),response=s))
...
class AlpacaSupervisedDataset(Dataset):
    def __init__(self,tokenizer,num_examples,lorra_args,):
        ...
        orig_s,pos_s,neg_s=get_truncated_outputs(
            ...
            pos_type=lorra_args.pos_type,
            neg_type=lorra_args.neg_type,
            control_template=lorra_args.control_template
            )
        self.orig_s=orig_s
        self.pos_s=pos_s
        self.neg_s=neg_s
    ...
    def __getitem__(self,i):
        ...
        #loss_mask 사용 
        texts=[orig_s,pos_s,neg_s]
        self.tokenizer.padding_side="left"
        encodings=self.tokenizer(
            texts,
            padding="max_length",
            truncation=True,
            max_length=self.max_res_len+256,
            return_tensors="pt",
        )
        ...
        for idx,text in enumerate(texts):
            parts=text.split(assistant_tag)
            if len(parts)>1:
                response_part=parts[1]
            else:
                print("assistant_tag 감지 실패")
                raise ValueError(f"text:{text[:100]}")
            self.tokenizer.padding_side="right"
            res_tokens=self.tokenizer(response_part,truncation=True,max_length=self.max_res_len)["input_ids"]
            res_len=len(res_tokens)
            self.tokenizer.padding_side="left"
            loss_mask[idx,-res_len:]=1
        return dict(
            input_ids=input_ids,
...
def get_logprobs_accuracy(model,tokenizer,questions,answers,labels,batch_size):
    ...
    for i in range (len(questions)//batch_size+1):
        ...
        with torch.no_grad():
            ...
            logprobs=get_logprobs(logits,inputs["input_ids"],masks).sum(-1).detach().float().cpu().numpy()
