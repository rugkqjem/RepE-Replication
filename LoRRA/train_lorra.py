...
def compute_loss(self,model,inputs,target_layers,alpha,beta,max_res_len,return_outputs=False,**kwargs):
    ...
    final_mask = loss_mask[:, 0].unsqueeze(0).repeat(len(target_layers), 1, 1).unsqueeze(-1)
    with model.disable_adapter():
        ...
        with torch.no_grad():
            ...
            orig_hidden=[orig_outputs[l].detach() for l in target_layers]
            pos_outputs=model(
            ...
            direction_hidden=[pos_outputs[l].detach() - neg_outputs[l].detach() for l in target_layers]
            target_hidden=torch.stack([orig_hidden[i]+alpha*direction_hidden[i] for i in range(len(target_layers))])
            target_hidden = target_hidden * final_mask

            diff_norm = torch.norm(direction_hidden[0], p=2).item()
    ...
    lora_hidden=torch.stack([lora_outputs[l] for l in target_layers])*final_mask
...
def train():
    parser=transformers.HfArgumentParser(
        (ModelArguments,MyTrainingArguments,LoraArguments,LorraArguments)
    )
    (
        model_args,
        training_args,
